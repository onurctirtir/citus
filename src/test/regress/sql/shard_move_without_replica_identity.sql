...
-- transfer mode, 'force_advanced_logical'. With any other mode the behavior is
-- identical to upstream Citus. When 'force_advanced_logical' is used, Citus does
-- all three of the following for a no-replica-identity table:
--
...
--    publishable, so 'force_advanced_logical' can transfer such a table with a
--    non-blocking move.
--
...
--
-- SECTION 1: any mode other than force_advanced_logical => upstream behavior is
--            preserved.
--
...
--     touch the replica identity (that is only done by force_advanced_logical), so
...
--
-- SECTION 2: force_advanced_logical transfers a no-replica-identity table with a
--            non-blocking move and faithfully restores its original replica
--            identity afterwards.
--

--
-- 2a) no replica identity + NO index, force_advanced_logical: the source is set to
...
SELECT citus_move_shard_placement(:shardid_no_idx2, 'localhost', :src_no_idx2, 'localhost', :tgt_no_idx2, shard_transfer_mode:='force_advanced_logical');
SELECT public.wait_for_resource_cleanup();
SELECT count(*) FROM t_no_idx;
SELECT DISTINCT result FROM run_command_on_placements('t_no_idx', 'SELECT relreplident FROM pg_class WHERE oid = ''%s''::regclass');

--
-- 2b) no replica identity + a usable btree index, force_advanced_logical: also
...
SELECT citus_move_shard_placement(:shardid_usable2, 'localhost', :src_usable2, 'localhost', :tgt_usable2, shard_transfer_mode:='force_advanced_logical');
SELECT public.wait_for_resource_cleanup();
...
--
-- 2c) REPLICA IDENTITY NOTHING + a usable index, force_advanced_logical: succeeds,
...
SELECT citus_move_shard_placement(:shardid_nothing, 'localhost', :src_nothing, 'localhost', :tgt_nothing, shard_transfer_mode:='force_advanced_logical');
SELECT public.wait_for_resource_cleanup();
...
--
-- SECTION 3: under force_advanced_logical one existing usable index is built early
...
--
-- 3a) no replica identity + a usable btree index, force_advanced_logical: the
...
SELECT citus_move_shard_placement(:shardid_early, 'localhost', :src_early, 'localhost', :tgt_early, shard_transfer_mode:='force_advanced_logical');
SELECT public.wait_for_resource_cleanup();
...
SELECT citus_move_shard_placement(:shardid_epartial, 'localhost', :src_epartial, 'localhost', :tgt_epartial, shard_transfer_mode:='force_advanced_logical');
SELECT public.wait_for_resource_cleanup();
...
--
-- SECTION 4: under force_advanced_logical a throwaway helper btree index is built
--            on the destination shard of a table that has no usable index, and
--            dropped when the move finishes.
--

--
-- 4a) no replica identity + no index, force_advanced_logical: the helper index is
...
--     force_advanced_logical also sets REPLICA IDENTITY FULL on the source for the
...
SELECT citus_move_shard_placement(:shardid_temp, 'localhost', :src_temp, 'localhost', :tgt_temp, shard_transfer_mode:='force_advanced_logical');
SELECT public.wait_for_resource_cleanup();
-- data preserved
...
--     force_advanced_logical a helper index is built (and dropped afterwards) for it
...
SELECT citus_move_shard_placement(:shardid_tempfull, 'localhost', :src_tempfull, 'localhost', :tgt_tempfull, shard_transfer_mode:='force_advanced_logical');
SELECT public.wait_for_resource_cleanup();
-- data preserved
...
--
-- SECTION 5: force_advanced_logical is a shard-MOVE-only capability. It only
...
-- citus_split_shard_by_split_points rejects force_advanced_logical
SELECT citus_split_shard_by_split_points(:shardid_reject, ARRAY['-1500000000'], ARRAY[1, 2], 'force_advanced_logical');

-- isolate_tenant_to_new_shard (tenant isolation) rejects force_advanced_logical
SELECT isolate_tenant_to_new_shard('reject_split', 5, shard_transfer_mode:='force_advanced_logical');
