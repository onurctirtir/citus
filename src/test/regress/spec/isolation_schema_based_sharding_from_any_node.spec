...
step "coord-show-tables-in-schema-sc1"
{
	...
	FROM run_command_on_all_nodes($$
		SELECT array_agg(tablename ORDER BY tablename) FROM pg_tables WHERE schemaname = 'sc1' AND tablename IN ('t1', 't2')
	$$)
...
// TODO: - test with explicit create_reference_table in xact
