...
RegisterCitusConfigVariables(void)
{
	...
	DefineCustomIntVariable(
		...
		gettext_noop("While activating a node, Citus opens each distributed "
					 "relation to build its shell table and metadata commands, "
					 "which accumulates PostgreSQL relcache/catcache and Citus "
					 "metadata cache entries on the coordinator. Citus flushes "
					 "these caches once every this many objects so that syncing "
					 "metadata for a very large number of distributed tables does "
					 "not exhaust coordinator memory. Setting this to 0 disables "
					 "the periodic flushing."),
